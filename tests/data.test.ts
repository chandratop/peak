...
test('shipped data validates and weights include quantities', () => {
  ...
  assert.equal(parseGear(gear).length, 52);
  assert.equal(totalWeight([{ weight_g: 100, qty: 3 }]), 300);
});
test('invalid CSV enums, quantities, weights and headers are rejected', () => {
  for (const bad of [gear.replace('580,1', '580,0'), gear.replace('1482,1', '-1,1'), gear.replace('pending,critical', 'confirmed,critical'), gear.replace('weight_g', 'weight')]) assert.throws(() => parseGear(bad));
});
