...
const placements = ['yes', 'no', 'just one'];
...
export function parseGear(text: string): GearItem[] {
  ...
  const required = ['item_name', 'category', 'weight_g', 'qty', 'status', 'priority', 'not_in_rucksack'];
  ...
  return result.data.map((row, index) => {
    ...
    if (!row.item_name?.trim() || !categories.includes(row.category) || !row.weight_g?.trim() || !finite(weight) || weight < 0 || !row.qty?.trim() || !Number.isInteger(qty) || qty < 1 || !['packed', 'pending'].includes(row.status) || !['critical', 'optional'].includes(row.priority) || !placements.includes(row.not_in_rucksack)) throw new Error(`Invalid gear values on row ${index + 2}`);
