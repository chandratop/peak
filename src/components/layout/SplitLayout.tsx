...
export default function SplitLayout({ children }: SplitLayoutProps) {
  ...
  return (
    <MapContext.Provider value={{ mapRef, mapStyle, setMapStyle }}>
      {/* Root: relative so panel can overlay the map */}
      <div className="relative h-screen w-screen overflow-hidden bg-amoled min-w-[1024px]">

        {/* Map — always fills the full viewport, never resizes */}
        <div className="absolute inset-0">
          {children[1]}
        </div>

        {/* Panel — floats over the map as an absolute overlay */}
        <div
          className={`
          ...
          `}
        >
          {/* Scrollable content — fade out when collapsing to prevent text flash */}
          <div
            className={`
          ...
          {/* Expand / minimize button — top-right corner of panel */}
          ...
          {/* Collapse tab — hangs off the right edge into the map */}
          <button
